...
namespace ICCEmbedded.SharpZipLib.Zip
{
    ...
    public class FastZip
    {
        ...
        public INameTransform NameTransform
        {
            ...
            set { entryFactory.NameTransform = value; }
        }
        ...
        public IEntryFactory EntryFactory
        {
            ...
            set {
                entryFactory = value ?? new ZipEntryFactory();
            }
        }
        ...
        {
            get { return restoreDateTimeOnExtract; }
            set { restoreDateTimeOnExtract = value; }
        }
        ...
        private void ExtractFileEntry(ZipEntry entry, string targetName)
        {
            ...
            if (proceed)
            {
                ...
                if (continueRunning)
                {
                        ...
                        using (FileStream outputStream = File.Create(targetName))
                        {
                            ...
                            if ((events != null) && (events.Progress != null))
                            {
                                StreamUtils.Copy(zipFile.GetInputStream(entry), outputStream, buffer,
                                                 events.Progress, events.ProgressInterval, this, entry.Name);
                            }
                            else
                            {
                                StreamUtils.Copy(zipFile.GetInputStream(entry), outputStream, buffer);
                    ...
                    {
                        continueRunning = events != null && events.OnFileFailure(targetName, ex);
                    }
                }
            }
        }
        ...
        private void ExtractEntry(ZipEntry entry)
        {
            ...
            if (doExtraction)
            {
                ...
                {
                    targetName = Path.Combine(targetDirectory, nameText);
            ...
            if (doExtraction && !Directory.Exists(dirName))
            {
                if (!entry.IsDirectory || CreateEmptyDirectories)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (events != null)
                        {
                            continueRunning = entry.IsDirectory ? events.OnDirectoryFailure(targetName, ex) : events.OnFileFailure(targetName, ex);
                        }
                        else
                        {
